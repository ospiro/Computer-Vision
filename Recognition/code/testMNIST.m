...
featureTypes = {'hog'};%'pixel', 'hog', 'lbp'};
...
for i = 1:length(dataTypes), 
    ...
    for j = 1:length(featureTypes),
        ...
        % Train model
        tic;
%         B=TreeBagger(100,features(:, data.set==trainSet)', data.y(data.set==trainSet)');
        ...
        % Test the model
%         ypred = str2num(cell2mat(B.predict(features(:,data.set==testSet)')))';
