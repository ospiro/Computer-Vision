...
% Global variables
maxShift = [-15 -15;15 15]; % the shifts are checked within this range
display = false;     % if set to true then aligned images are shown
...
for i = 1:numImages,
    ...
    [channels, gtShift]  = randomlyShiftChannels(im, maxShift(2,:));
    
    % Compute alignment
    smallIm = imresize(channels,0.25);
    [colorIm, predShift] = alignChannelsCoarsetoFine(smallIm, round(maxShift/4));
    for k = 1:4
        if predShift(1,1
    [colorIm, predShift] = alignChannelsCoarsetoFine(im, [fineShiftMinI fineShiftMinJ;fineShiftMaxI fineShiftMaxJ]);
    % Print results
